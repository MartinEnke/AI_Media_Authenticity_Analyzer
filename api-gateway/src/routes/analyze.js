...
async function saveUploadedFile(file) {
  ...
  const filePath = path.join(uploadsDir, `${requestId}-${file.filename}`);
  ...
  return { requestId, filePath, filename: file.filename, mimetype: file.mimetype };
}
...
async function routes(fastify) {
  ...
  for await (const part of parts) {
    if (part.type === "file" && part.fieldname === "file") {
      uploadedFile = part;
    } else if (part.type === "field" && part.fieldname === "claim") {
      claim = part.value;
    }
  }
